...
class Distribution:
    ...
    def composed_distribution(self):
        """
...
def compute_sensitivity_algo_Saltelli(distribution, metamodel, sensitivity_experiment_size):
    ...
    """
    composed_distribution = distribution.composed_distribution()
...
def compute_and_export_sensitivity_algo_Saltelli(
    ...
    """
    complete_pkl_filename_metamodel = miu.create_pkl_name(type_of_metamodel + '_mechanoadaptation_elliptic', training_amount)
    ...
    complete_pkl_filename_sensitivy_algo = miu.create_pkl_name_sensitivityalgo(
        type_of_metamodel + '_mechanoadaptation_elliptic', training_amount, sensitivity_experiment_size, type_of_Sobol_sensitivity_implementation
    )
...
def get_indices_and_confidence_intervals(type_of_metamodel, training_amount, sensitivity_experiment_size, type_of_Sobol_sensitivity_implementation
    ...
    complete_pkl_filename_sensitivy_algo = miu.create_pkl_name_sensitivityalgo(type_of_metamodel + '_mechanoadaptation_elliptic'
    ...
    first_order_indices_confidence_lowerbounds = [
        first_order_indices_confidence_interval.getLowerBound()[k] for k in [0, 1, 2, 3]
    ]
    first_order_indices_confidence_upperbounds = [
        first_order_indices_confidence_interval.getUpperBound()[k] for k in [0, 1, 2, 3]
    ]
    ...
    total_order_indices_confidence_lowerbounds = [
        total_order_indices_confidence_interval.getLowerBound()[k] for k in [0, 1, 2, 3]
    ]
    total_order_indices_confidence_upperbounds = [
        total_order_indices_confidence_interval.getUpperBound()[k] for k in [0, 1, 2, 3]
    ]
    first_order_indices_confidence_errorbar = np.zeros((2, 4))
    total_order_indices_confidence_errorbar = np.zeros((2, 4))
    for k in range(4):
...
def gradient(vector):
    ...
    for k in range(nb_samples-1):
        gradient[0, k] = np.abs(np.diff(vector)[k])# / vector[k]
    return gradient
...
def compute_convergence_Sobol_indices(
    ...
    Saltelli_first_order_indices_vs_experiment_size = np.zeros((4, len(sensitivity_experiment_size_list)))
    Saltelli_total_order_indices_vs_experiment_size = np.zeros_like(Saltelli_first_order_indices_vs_experiment_size)
    Saltelli_first_order_indices_confidence_errorbars_vs_experiment_size = np.zeros((2, 4, len(sensitivity_experiment_size_list)))
    ...
    miu.export_utils_sensitivity_convergence_confinterval("sensitivity_analysis_convergence_confinterval_mechanoadaptation_elliptic.pkl", Saltelli_first_order_indices_vs_experiment_size_confinterval,Saltelli_total_order_indices_vs_experiment_size_confinterval,Jansen_first_order_indices_vs_experiment_size_confinterval,Jansen_total_order_indices_vs_experiment_size_confinterval,MauntzKucherenko_first_order_indices_vs_experiment_size_confinterval,MauntzKucherenko_total_order_indices_vs_experiment_size_confinterval,Martinez_first_order_indices_vs_experiment_size_confinterval,Martinez_total_order_indices_vs_experiment_size_confinterval)

    Saltelli_first_order_indices_vs_experiment_size_gradient = np.zeros((4, len(sensitivity_experiment_size_list)-1))
...
def compute_sensitivity_indices_PCE(training_amount, degree):
    ...
    first_order_indices = [chaosSI.getSobolIndex(k) for k in range(4)]
    total_order_indices = [chaosSI.getSobolTotalIndex(k) for k in range(4)]
...
computation_time_vs_experiment_size,     createfigure,
    ...
    labels = [ r"$\overline{\gamma}_A$", r"$\overline{\gamma}_D$", r"$\overline{\gamma}_S$", r"$\overline{r}$"]
    palette = sns.color_palette("Paired")
    line_colors = [palette[k] for k in range(5, len(palette), 2)]
    ...
    savefigure.save_as_png(fig_first, 'convergence_firstSobol_mechanoadaptation_elliptic_' +  type_of_Sobol_sensitivity_implementation + str(pixels) + "p")
    savefigure.save_as_png(fig_total, 'convergence_totalSobol_mechanoadaptation_elliptic_' +  type_of_Sobol_sensitivity_implementation + str(pixels) + "p")
...
def plot_gradient_Sobol_vs_sample_size(sensitivity_experiment_size_list):
    ...
    """
    filename_sensitivity_convergence_gradient = "sensitivity_analysis_convergence_gradient_mechanoadaptation_elliptic.pkl"
    ...
    savefigure.save_as_png(fig_Saltelli_first, "gradient_firstSobol_Saltelli_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_Saltelli_total, "gradient_totalSobol_Saltelli_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_Jansen_first, "gradient_firstSobol_Jansen_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_Jansen_total, "gradient_totalSobol_Jansen_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_MauntzKucherenko_first, "gradient_firstSobol_MauntzKucherenko_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_MauntzKucherenko_total, "gradient_totalSobol_MauntzKucherenko_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_Martinez_first, "gradient_firstSobol_Martinez_mechanoadaptation_elliptic")
    ...
    savefigure.save_as_png(fig_Martinez_total, "gradient_totalSobol_Martinez_mechanoadaptation_elliptic")
...
def plot_confinterval_Sobol_vs_sample_size(sensitivity_experiment_size_list):
    ...
    """
    filename_sensitivity_convergence_confinterval = "sensitivity_analysis_convergence_confinterval_mechanoadaptation_elliptic.pkl"
    ...
    for i in range(4):
        ax_Saltelli_first.plot(
            ...
            lw=2)
    ax_Saltelli_first.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_Saltelli_first.set_ylabel(r"Conf. intervals of $S_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_Saltelli_first, "confinterval_firstSobol_Saltelli_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_Saltelli_total.plot(
            ...
            lw=2)
    ax_Saltelli_total.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_Saltelli_total.set_ylabel(r"Conf. intervals of $ST_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_Saltelli_total, "confinterval_totalSobol_Saltelli_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_Jansen_first.plot(
            ...
            lw=2)
    ax_Jansen_first.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_Jansen_first.set_ylabel(r"Conf. intervals of $S_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_Jansen_first, "confinterval_firstSobol_Jansen_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_Jansen_total.plot(
            ...
            lw=2)
    ax_Jansen_total.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_Jansen_total.set_ylabel(r"Conf. intervals of $ST_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_Jansen_total, "confinterval_totalSobol_Jansen_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_MauntzKucherenko_first.plot(
            ...
            lw=2)
    ax_MauntzKucherenko_first.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_MauntzKucherenko_first.set_ylabel(r"Conf. intervals of $S_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_MauntzKucherenko_first, "confinterval_firstSobol_MauntzKucherenko_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_MauntzKucherenko_total.plot(
            ...
            lw=2)
    ax_MauntzKucherenko_total.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_MauntzKucherenko_total.set_ylabel(r"Conf. intervals of $ST_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_MauntzKucherenko_total, "confinterval_totalSobol_MauntzKucherenko_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_Martinez_first.plot(
            ...
            lw=2)
    ax_Martinez_first.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_Martinez_first.set_ylabel(r"Conf. intervals of $S_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_Martinez_first, "confinterval_firstSobol_Martinez_mechanoadaptation_elliptic")
    ...
    for i in range(4):
        ax_Martinez_total.plot(
            ...
            lw=2)
    ax_Martinez_total.legend(prop=fonts.serif(), loc="upper right", framealpha=0.7)
    ax_Martinez_total.set_ylabel(r"Conf. intervals of $ST_i$ [ - ]", font=fonts.serif(), fontsize=fonts.axis_label_size())
    savefigure.save_as_png(fig_Martinez_total, "confinterval_totalSobol_Martinez_mechanoadaptation_elliptic")
...
def plot_comparison_indices(
    ...
    ax.set_xticklabels(
        [r"$\overline{\gamma}_A$", r"$\overline{\gamma}_D$", r"$\overline{\gamma}_S$", r"$\overline{r}$"],
        font=fonts.serif(),
        fontsize=fonts.axis_legend_size(),
    )
    ...
    filename = (
        ...
        + "_mechanoadaptation_elliptic_trainingamount="
        + str(training_amount)
        + "_size="
...
def plot_comparison_indices_piechart(
    ...
    palette = sns.color_palette("Paired")
    colors = (palette[k] for k in range(5, len(palette), 2))
    labels = [ r"$\overline{\gamma}_A$", r"$\overline{\gamma}_D$", r"$\overline{\gamma}_S$", r"$\overline{r}$"]
    fig = createfigure.square_figure_7(pixels=pixels)
    ax = fig.gca()
    ...
    filename = ('sobolindices_Kriging_piechart_mechanoadaptation_elliptic')
    savefigure.save_as_png(fig, filename + str(pixels) + "p")


if __name__ == "__main__":
    type_of_metamodel = "Kriging"
    training_amount = 0.65
    ...
    compute_convergence_Sobol_indices(
        ...
        createfigure,
        pixels)
    filename_sensitivity_convergence = "sensitivity_analysis_convergence_mechanoadaptation_elliptic.pkl"
